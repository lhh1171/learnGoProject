...
func Myfile5() {
	mygetFilelist("/MyProject/goProject/src/base5_io")
}

func mywritefile() {
	userFile := "/MyProject/goProject/src/base5_io/xx/myfile.txt" //文件路径
...
func mycopyfile() {
	fi, err := os.Open("/MyProject/goProject/src/base5_io/xx/input.txt") //打开输入*File
	if err != nil {
		panic(err)
	}
	defer fi.Close()

	fo, err := os.Create("/MyProject/goProject/src/base5_io/xx/output.txt") //创建输出*File
	if err != nil {
		panic(err)
	}
	defer fo.Close()
...
func myioutilcopyfile() {
	b, err := ioutil.ReadFile("/MyProject/goProject/src/base5_io/xx/input.txt") //读文件
	if err != nil {
		panic(err)
	}

	err = ioutil.WriteFile("/MyProject/goProject/src/base5_io/xx/output.txt", b, 0644) //写文件
	if err != nil {
		panic(err)
	}
}
