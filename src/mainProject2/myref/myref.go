...
func Myref3() {
	...
	for i := 0; i < tt.NumMethod(); i++ { //仅能获取非指针对象的引用方法
		m := tt.Method(i)
		fmt.Println(m.Name, m.Type)
	}
}
...
func Myref4() {
	s := student{person: person{int: 1, name: "aaa"}, age: 22}
	t := reflect.TypeOf(s)
	fmt.Println(t.FieldByName("person")) //返回值第二个代表是不是匿名字段
...
func Myref6() {
	// TypeOf
	str := "this is string"
	fmt.Println(reflect.TypeOf(str), reflect.TypeOf(str).Kind()) // output: "string"
	...
	fmt.Println(typ, typ.Kind(), typ.Elem().Kind()) // output: "*myref.person"
	...
	cha := make(<-chan int)
	fmt.Println(reflect.TypeOf(cha), "|", reflect.TypeOf(cha).Kind(), "|", reflect.TypeOf(cha).ChanDir()) // chan
	cha1 := make(chan<- int)
	fmt.Println(reflect.TypeOf(cha1).ChanDir()) // chan

	cha2 := make(chan int)
	fmt.Println("chan2", reflect.TypeOf(cha2).ChanDir()) // chan
